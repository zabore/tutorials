...
The {ggsurvfit} package works best if you create the `survfit` object using the included `ggsurvfit::survfit2()` function, which uses similar syntax to what we saw previously with `survival::survfit()`.
...
Typically we will also want to see the numbers at risk in a table below the x-axis. We can add this using `add_risk_table()`:
...
If we want to include both event types, we can call the `autoplot()` function from the {tidycmprsk} package:
