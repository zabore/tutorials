...
3. (Advanced) Clone this remote repository to a local directory, replacing step 1 below
...
- The page will be added as to the page: YOUR_GH_NAME.github.io/repo_name
    ...
    - Each function can be clicked through to see the help page
...
1. Create a Github directory named YOUR_GH_NAME.github.io, where YOUR_GH_NAME is your GitHub username, initialized with a README file
2. Clone the Github directory to a local directory with the same name
...
5. Choose a [theme](https://themes.gohugo.io/) and find the link to the theme's GitHub repository
...
*Problem 1*: Because it is a static site, upon building, the files needed to generate the site online are automatically created in a separate subdirectory within your local directory. However this will cause problems with GitHub hosting since the files to host need to be in the YOUR_GH_NAME.github.io folder.

My solution:

1. Maintain separate folders for the source files (I named this folder "source") and for the static files (the folder YOUR_GH_NAME.github.io). The "source" folder is where your R project and `config.toml` files will live.
...
*Problem 2*: GitHub defaults to using Jekyll with website content, and this needs to be disabled since `blogdown` sites are built with Hugo (don't worry if you don't understand this, just heed my words!)
...
- [http://hadley.github.io/pkgdown/](http://hadley.github.io/pkgdown/): Hadley Wickham's `pkgdown` website
